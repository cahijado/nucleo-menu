...
export function generateMenu(cfg, recipes, ingIndex, pinned = {}) {
  ...
  for (let d = 0; d < cfg.dias; d++) {
    ...
    /* Kcal de cada toma ya elegida hoy, y con qué se podría rebajar si el
       día se pasa del objetivo (más abajo). Solo llevan entrada las tomas
       que elige el generador: una fijada con el candado no se toca, y una
       sin plato no suma nada. */
    const kcalDia = {};
    const candidatosPorToma = {};
    const usadosIdx = {};
    const historialIdx = {};
    const protDeToma = {};
    ...
    for (const toma of cfg.tomas) {
      ...
      if (laFijada) {
        ...
        kcalDia[toma] = recipeNutrition(laFijada, ingIndex).kcal;
      ...
      /* Ya no se puntúa por acercarse a un reparto de kcal por toma: con
         comida y cena clasificadas por su propio aporte (comida generosa,
         cena hasta 500 kcal), pedir además que cada plato encajara en una
         fracción exacta del objetivo diario solo estrechaba el recetario a
         los platos más calóricos que cupieran ahí — el motivo real de la
         poca variedad. El reparto sale libre, con toda la variedad del
         recetario; el objetivo del perfil se vigila una vez armado el día
         entero, un poco más abajo. */
      const puntuados = lista.map((r) => {
        const n = recipeNutrition(r, ingIndex);
        let s = 0;
      ...
      const mejor = puntuados[0].r;
      platos[toma] = mejor.id;
      kcalDia[toma] = puntuados[0].n.kcal;
      candidatosPorToma[toma] = puntuados;
      huecos = Math.max(0, huecos - 1);

      usadosIdx[toma] = usados.length;
      usados.push({ id: mejor.id, dia: d });
      historialIdx[toma] = historial.length;
      historial.push({ d, rasgos: rasgosDe(mejor) });
      const prot = mainProtein(mejor, ingIndex);
      protDeToma[toma] = prot || null;
      if (prot) proteinasSemana[prot] = (proteinasSemana[prot] || 0) + 1;
    }

    /* El único sitio donde sí importan las kcal del día entero: si las tomas
       elegidas hoy suman más que el objetivo del perfil, se rebaja la que
       más pese, cambiándola por la siguiente mejor puntuada de su propia
       lista que aporte menos — nunca la primera que se cruce, la mejor
       posible entre las que ya aportan menos. Se repite hasta encajar o
       hasta que ninguna toma tenga ya más margen (una fijada con el candado
       nunca se toca: no tiene lista de candidatos). */
    let totalKcal = Object.values(kcalDia).reduce((a, k) => a + k, 0);
    let guardia = 0;
    while (totalKcal > targets.kcal && guardia++ < 30) {
      let peor = null;
      for (const toma of Object.keys(candidatosPorToma)) {
        if (peor === null || kcalDia[toma] > kcalDia[peor]) peor = toma;
      }
      if (!peor) break;

      const otrasHoy = Object.entries(platos).filter(([t]) => t !== peor).map(([, rid]) => rid);
      const alternativa = candidatosPorToma[peor].find(
        (c) => c.n.kcal < kcalDia[peor] && !otrasHoy.includes(c.r.id)
      );
      if (!alternativa) { delete candidatosPorToma[peor]; continue; }

      if (protDeToma[peor]) proteinasSemana[protDeToma[peor]] -= 1;
      const nuevoProt = mainProtein(alternativa.r, ingIndex);
      protDeToma[peor] = nuevoProt || null;
      if (nuevoProt) proteinasSemana[nuevoProt] = (proteinasSemana[nuevoProt] || 0) + 1;

      usados[usadosIdx[peor]] = { id: alternativa.r.id, dia: d };
      historial[historialIdx[peor]] = { d, rasgos: rasgosDe(alternativa.r) };
      platos[peor] = alternativa.r.id;
      totalKcal += alternativa.n.kcal - kcalDia[peor];
      kcalDia[peor] = alternativa.n.kcal;
    }
